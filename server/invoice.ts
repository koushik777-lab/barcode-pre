...
const autoTable = (_autoTable as any).default || _autoTable;
...
export const generateInvoicePDF = async (order: Order, user: { username: string }) => {
  // Use 'p' for portrait, 'mm' for millimeters, 'a4' for size
  const doc = new jsPDF();
  ...
  // Helper for adding images in Node environment
  const addImageToDoc = (filePath: string, type: string, x: number, y: number, w: number, h: number) => {
    try {
      if (fs.existsSync(filePath)) {
        const imgData = fs.readFileSync(filePath);
        const base64Img = imgData.toString("base64");
        doc.addImage(`data:image/${type === "PNG" ? "png" : "jpeg"};base64,${base64Img}`, type, x, y, w, h);
      }
    } catch (e) {
      console.warn(`Asset not found for invoice: ${filePath}`, e);
    }
  };

  // Add Logo
  addImageToDoc(path.join(publicPath, "new_logo.jpeg"), "JPEG", 15, 10, 25, 25);
  ...
  const estNo = `MSR-${createdAt.getFullYear()}-${(createdAt.getFullYear() + 1).toString().slice(-2)}/${order._id.toString().slice(-4).toUpperCase()}`;
  
  doc.text(`Estimate No. : ${estNo}`, pageWidth - 20, 62, { align: "right" });
  doc.text(`Date : ${dateStr}`, pageWidth - 20, 68, { align: "right" });
  doc.text(`Order ID : ${order._id.toString().toUpperCase()}`, pageWidth - 20, 74, { align: "right" });
  ...
  // Add Signature
  addImageToDoc(path.join(publicPath, "signature.png"), "PNG", pageWidth - 55, sigY - 5, 40, 15);
  ...
  // Output as array buffer and convert to Buffer
